...
for ax, stimulus in zip(axs.flatten(), natimgs_normed[:9]):
...
for idx, (small_stimulus, big_stimulus) in enumerate(zip(small_stimuli, natimgs_normed)):
    print(f"Processing stimulus {idx + 1}/{len(small_stimuli)}")
    
    idata = model(
        image=small_stimulus,
        n_samples=n_draws,
        tune=n_tune,
        chains=n_chains,
        cores=n_cores,
    ...
    with open(f"results/coen_cagli_small_stimulus_{idx}.pkl", "wb") as f:
        pickle.dump(idata, f)
